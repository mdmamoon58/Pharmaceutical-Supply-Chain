...
contract Distributor {
    ...
    mapping(address => bool) private registeredDistributors;
    ...
    modifier onlyRegisteredDistributor() {
        require(registeredDistributors[msg.sender], "Only registered Distributors can call this function");
        _;
    }
    ...
    function registerDistributor() external {
        registeredDistributors[msg.sender] = true;
    }

    function unregisterDistributor() external {
        registeredDistributors[msg.sender] = false;
    }

    function addProduct(string memory name, uint256 price, uint256 quantity) external onlyRegisteredDistributor {
    ...
    function removeProduct(string memory name) external onlyRegisteredDistributor {
        require(products[name].available, "Product does not exist");

        delete products[name];
    }

    function isDistributorRegistered(address distributor) external view returns (bool) {
        return registeredDistributors[distributor];
    }
    ...
    function buyProduct(string memory name, uint256 quantityToBuy) external payable {
        ...
        // For simplicity, let's assume the Distributor is transferring the product directly to the Distributor's address
